...
{
  /* See [IA32-v2a] "INS". */
  __asm volatile ("rep insb" : "+D" (addr), "+c" (cnt) : "d" (port) : "memory");
}
...
inw (uint16_t port)
{
  uint16_t data;
  /* See [IA32-v2a] "IN". */
  __asm volatile ("inw %w1, %w0" : "=a" (data) : "Nd" (port));
  return data;
}
...
{
  /* See [IA32-v2a] "INS". */
  __asm volatile ("rep insw" : "+D" (addr), "+c" (cnt) : "d" (port) : "memory");
}
...
{
  /* See [IA32-v2a] "INS". */
  __asm volatile ("rep insl" : "+D" (addr), "+c" (cnt) : "d" (port) : "memory");
}
...
{
  /* See [IA32-v2b] "OUT". */
  __asm volatile ("outw %w0, %w1" : : "a" (data), "Nd" (port));
}
